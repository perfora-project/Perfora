...
class LaneFinding:
    ...
    def run(self, ctx: PipelineContext) -> None:
        ...
        d = v_density(ctx.holes, n_cols, cfg.density_sigma_px)
        ctx.debug["v_density"] = d

        # --- pitch (px): override, else windowed consensus + global refine ---
        method = "comb-fit"
        if ov.lane_pitch_mm is not None:
            pitch_px = ov.lane_pitch_mm / mm_v
            confidence = 1.0
            method = "override"
        else:
            pitch0, conf_w = _windowed_pitch(ctx.holes, n_rows, n_cols, cfg)
            if not np.isfinite(pitch0):
                pitch0, conf_g, method = estimate_pitch(
                    d,
                    min_pitch_px=cfg.min_pitch_px,
                    max_pitch_px=cfg.max_pitch_px,
                    min_freq_bin=cfg.min_freq_bin,
            ...
            pitch_px, _v0_unused = comb_fit(d, pitch0)

        # --- offset v0 (px): override or comb fit ---
        ...
        idx = np.round((cols - v0_raw) / pitch_px).astype(int)
        min_idx = int(idx.min())
        max_idx = int(idx.max())
